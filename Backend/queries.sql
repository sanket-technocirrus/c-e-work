...
--     points INT,
--     FOREIGN KEY (question_id) REFERENCES questions_tbl(question_id)
-- );

-- for adding ON DELETE CASCADE constraint to test_cases_tbl 
-- cz if question is deleted from db its testcases will also be deleted from db
-- since i didn't give name to foreign key constraint while creating it, db system gave it automatically i.e test_cases_tbl_ibfk_1
ALTER TABLE test_cases_tbl
DROP FOREIGN KEY test_cases_tbl_ibfk_1;

-- to get that constraint name run below query
SELECT CONSTRAINT_NAME
FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE
WHERE TABLE_NAME = 'test_cases_tbl' AND COLUMN_NAME = 'question_id';

-- modified test_cases_tbl
ALTER TABLE test_cases_tbl 
ADD CONSTRAINT fk_question_id -- here i gave name -> fk_question_id
